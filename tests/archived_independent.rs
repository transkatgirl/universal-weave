...
impl StateMachineTest for WeaveWrapper {
    ...
    fn apply(
        ...
        state
            .weave
            .get_ordered_identifiers_from(&state.target, &mut state.ordered_node_identifiers_from);
    ...
    fn check_invariants(
        ...
        assert!(
            state.weave.get(&state.target).map(|n| n.id)
                == archived
                    .get(&u32_le::from(state.target))
                    .map(|n| n.id.to_native())
        );
        archived.get_ordered_identifiers(&mut buffer);
        assert!(state.ordered_node_identifiers == buffer);
        archived.get_ordered_identifiers_from(&u32_le::from(state.target), &mut buffer);
