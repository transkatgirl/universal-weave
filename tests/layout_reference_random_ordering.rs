...
struct PrioritizedWeave {
    weave: IndependentWeave<u32, WeaveContent, u32, RandomState>,
    priorities: HashMap<u32, u32>,
}

impl Weave<u32, IndependentNode<u32, WeaveContent, RandomState>, WeaveContent>
    for PrioritizedWeave
{
    type Nodes = HashMap<u32, IndependentNode<u32, WeaveContent, RandomState>, RandomState>;
    type Roots = IndexSet<u32, RandomState>;
    ...
    fn contains_active(&self, id: &u32) -> bool {
        self.weave.contains_active(id)
    }
    fn get(&self, id: &u32) -> Option<&IndependentNode<u32, WeaveContent, RandomState>> {
        self.weave.get(id)
    }
    fn get_parents(&self, id: &u32) -> Option<&IndexSet<u32, RandomState>> {
        self.weave.get_parents(id)
    }
    fn get_children(&self, id: &u32) -> Option<&IndexSet<u32, RandomState>> {
        self.weave.get_children(id)
    }
    fn get_contents(&self, id: &u32) -> Option<&WeaveContent> {
        self.weave.get_contents(id)
    }
    fn get_ordered_identifiers(&mut self, output: &mut Vec<u32>) {
        output.clear();

        let mut remaining = HashMap::with_capacity(self.weave.len());

        remaining.extend(
            self.weave
                .nodes()
                .into_iter()
                .map(|(id, node)| (*id, node.from().len())),
        );
        ...
        while !ready.is_empty() {
            let id = ready.swap_remove(
                ready
                    .iter()
                    .copied()
                    .enumerate()
                    .min_by_key(|(_index, id)| (self.priorities[id], *id))
                    .unwrap()
                    .0,
            );
            ...
            for child in self.weave.get_children(&id).unwrap().into_iter().copied() {
    ...
    fn get_ordered_identifiers_from(&mut self, id: &u32, output: &mut Vec<u32>) {
        self.weave.get_ordered_identifiers_from(id, output);
    }
    fn get_active_path(&mut self, output: &mut Vec<u32>) {
        self.weave.get_active_path(output);
    }
    fn get_path_from(&mut self, id: &u32, output: &mut Vec<u32>) {
        self.weave.get_path_from(id, output);
    }
    fn insert(&mut self, node: IndependentNode<u32, WeaveContent, RandomState>) -> bool {
        self.weave.insert(node)
    }
    fn set_active(&mut self, id: &u32, value: bool) -> bool {
        self.weave.set_active(id, value)
    }
    fn remove(&mut self, id: &u32) -> Option<IndependentNode<u32, WeaveContent, RandomState>> {
        self.weave.remove(id)
    }
    fn remove_tracked(
        &mut self,
        id: &u32,
        on_removal: impl FnMut(IndependentNode<u32, WeaveContent, RandomState>),
    ) -> bool {
...
impl StateMachineTest for WeaveWrapper {
    ...
    fn init_test(
        ...
        WeaveWrapper {
            ...
            priorities: HashMap::with_capacity(ref_state.0.len()),
    ...
    fn apply(
        ...
        match transition.0 {
            WeaveTransition::Insert {
                ...
                state.priorities.insert(state.counter, priority);
            }
        ...
        let mut wrapped = PrioritizedWeave {
            weave: state.weave,
            priorities: state.priorities,
        };

        state.layouter.layout(&mut wrapped, |id| state.sizes[id]);
        state
            .reference_layouter
            .layout(&mut wrapped, |id| state.sizes[id]);

        state.weave = wrapped.weave;
        state.priorities = wrapped.priorities;
