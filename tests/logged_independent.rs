...
type InnerWeave = IndependentWeave<u32, WeaveContent, u32, RandomState>;
...
impl StateMachineTest for WeaveWrapper {
    ...
    fn init_test(
        ...
        //println!("init");
        WeaveWrapper {
            weave: LoggedWeave::with_capacity(
                IndependentWeave::with_capacity(ref_state.len(), ref_state.len() as u32),
                ref_state.len(),
            ),
            secondary_weave: IndependentWeave::with_capacity(
                ref_state.len(),
    ...
    fn apply(
        ...
        match transition.0 {
            ...
            WeaveTransition::Update => {
                while let Some(action) = state.weave.actions.pop_front() {
                    state.secondary_weave.apply(action);
                }

                assert_eq!(state.weave.weave, state.secondary_weave);
            }
        }
