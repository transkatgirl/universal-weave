    ...
    S: BuildHasher + Default + Clone,
{
    ...
    fn validate_subtree(
        ...
        if let Ok(meta) = tree.get_meta(target)
            ...
            && let Some(ValueOrContainer::Value(LoroValue::Binary(binary_contents))) =
                meta.get("contents")
            && let Ok(id) = from_bytes_aligned(&binary_id, buffer)
            && let Ok(contents) = from_bytes_aligned(&binary_contents, buffer)
            ...
            && node.contents == contents
        {
            *counter = counter.strict_add(1);
            ...
            for child in children {
                if !self.validate_subtree(tree, child, Some(id), buffer, counter) {
                    return false;
                }
            }
    ...
    fn validate(&self) -> bool {
        ...
        for (index, root) in tree.roots().into_iter().enumerate() {
            if let Some(at_index) = self.weave.roots.get_index(index)
                ...
                && self.validate_subtree(&tree, root, None, &mut buffer, &mut counter)
            {
            } else {
                return false;
            }
